...
export async function handler(event) {
  ...
  if (event.httpMethod === 'GET') {
    ...
    const { data, error } = await supabase
      .from('partes')
      .select('id, codigo, nombre, descripcion, unidad, costo, stock_actual, stock_minimo')
      .order('nombre');
    if (error) return response(500, { error: error.message });
    return response(200, { partes: data });
  }
  ...
  if (event.httpMethod === 'POST') {
    ...
    const { codigo, nombre, descripcion, unidad, costo, stock_actual, stock_minimo } = JSON.parse(event.body || '{}');
    if (!codigo || !nombre) return response(400, { error: 'codigo y nombre son requeridos' });
    const { data, error } = await supabase
      .from('partes')
      .insert({ codigo, nombre, descripcion, unidad: unidad || 'unidad', costo: costo || 0, stock_actual: stock_actual || 0, stock_minimo: stock_minimo || 0 })
      .select().single();
    if (error) return response(500, { error: error.message });
  ...
  if (event.httpMethod === 'PUT') {
    ...
    const allowed = ['codigo','nombre','descripcion','unidad','costo','stock_actual','stock_minimo'];

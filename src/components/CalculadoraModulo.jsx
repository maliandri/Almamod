...
// --- Datos Expandidos para la Calculadora ---
...
function CalculadoraModulo() {
  ...
  const availableRooms = useMemo(() => {
    return formData.size ? roomOptions[formData.size] : [];
  }, [formData.size]);

  const isStep1Valid = formData.size && formData.floor;
  const isStep2Valid = formData.roomCount && (!formData.hasBathroom || formData.bathroomQuality) && (!formData.hasKitchen || formData.kitchenQuality);
  
  const goToNextStep = () => {
    if (step === 2 && !formData.hasBathroom && !formData.hasKitchen) {
        calculateModuleProject();
    } else {
        setStep(s => s + 1);
    }
  }
              ...
              <div className="modal-body">
                ...
                {step === 1 && (
                  <motion.div className="calculator-step">
                    ...
                    <button onClick={() => setStep(2)} disabled={!isStep1Valid} className="main-action-button">Siguiente →</button>
                  </motion.div>
                )}
                
                {step === 2 && (
                    <motion.div className="calculator-step">
                        <h3>Paso 2: Distribución Interior</h3>
                        ...
                        {formData.size === '30' || formData.size === '45' ? (
                        ...
                        <div className="step-navigation">
                            ...
                            <button onClick={goToNextStep} disabled={!formData.roomCount} className="main-action-button">Siguiente →</button>
                        </div>
                    </motion.div>
                )}
                ...
                {step === 3 && (
                    <motion.div className="calculator-step">
                        ...
                        <div className="step-navigation">
                            ...
                            <button onClick={calculateModuleProject} disabled={!isStep2Valid} className="main-action-button">Calcular 🚀</button>
                        </div>
                    </motion.div>
                )}
                ...
                {step === 4 && results && (
                  <motion.div className="calculator-step results">
                    ...
                    <div className="final-price">${results.totalCost.toLocaleString('en-US')} USD</div>
                    ...
                    <p className="disclaimer">Costo aproximado. No incluye transporte o platea de hormigón.</p>
